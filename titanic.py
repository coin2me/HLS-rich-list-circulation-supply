...
while work:     
 ...
 counttx = outtx_num(chain)       # Get a number of send tx in this subchain. Returns 0 if error
 print('There are '+str(int(counttx,0))+' sent tx in this chain. Continue?')
 
 if int(counttx,0) > 0:           #PASS if there are no outcoming tx or if the current address contains error.
...
#    print('estimated tx num: '+str(int(counttx,0))+' done: '+str(found_tx)+' y count: '+str(y))
 ...
 if pp == len(wallets) - 1:          #STOP if the last processed / Normal STOP
  work = False
  supply = get_bals()
  dump_wallets(filename,balances)
  print('Total HLS:  '+str(supply))
  print('    ---- All WALLETS DETECTED ----')
 else:                              #go to the next subchain if there are more addresses
  pp = pp + 1
  chain = wallets[pp] 
